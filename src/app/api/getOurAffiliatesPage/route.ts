...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (populateParams.length > 0) {
            ...
            populateQuery = '&populate[0]=headingSection&populate[1]=headingSection.backgroundImg&populate[2]=SEO';
        }

        // Consultar el singleType our-affiliates-page
        const url = `${strapiUrl}/api/our-affiliates-page?${populateQuery.replace('&', '')}`;
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: 'Error al obtener la página de nuestros afiliados' },
        ...
        console.error('Error fetching our affiliates page:', error);
        return NextResponse.json({ error: "Failed to fetch our affiliates page" }, { status: 500 });
    }
}
