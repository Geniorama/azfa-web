...
export default function AffiliateStatisticsView({
  pageContent,
}: AffiliateStatisticsViewProps) {
  ...
  const defaultData = {
    ...
    smallTitle:
      "Explore visualizaciones interactivas con datos comparativos y tendencias clave de las zonas francas en Iberoamérica",
    disclaimerText: "",
    heroBackground: {
    ...
    ctaSection: {
      ...
      description:
        "Haga clic en el botón a continuación para acceder al formulario de edición. Recuerde ingresar con su usuario y contraseña asignados para continuar.",
      button: {
  ...
  const iframeData = Array.isArray(data.iframeCollection)
    ? data.iframeCollection
    : data.iframeCollection?.data || [];
              ...
              <div className="mt-12 space-y-8">
                {iframeData.map((iframeItem, index) => {
                  ...
                  const iframeContent =
                    ...
                    iframeItem.mobileIframe?.bottomText ||
                    "";
                  const iframeSrc = extractIframeSrc(iframeContent);
                  const iframeTitle =
                    iframeItem.desktopIframe?.title ||
                    iframeItem.mobileIframe?.title ||
                    "";
                  ...
                  return (
                    <div key={index} className="my-12">
                      <div className="mb-4">
                        <span className="text-button text-text-primary inline-block">
                          {iframeTitle}
                        </span>{" "}
                        <br />
